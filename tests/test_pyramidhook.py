...
from pyramid_rollers.pyramidhook import register_service_views
from pyramid_rollers.util import func_name, ContentTypePredicate, current_service
...
class TestServiceWithNonpickleableSchema(TestCase):
    def setUp(self):
        self.config = testing.setUp()
        self.config.registry.pyramid_rollers_services = {}
...
class TestFallbackRegistration(TestCase):
    ...
    def test_fallback_permission(self):
        ...
        for v in fallback_views:
            if v["introspectable"].title == "function pyramid_rollers.pyramidhook._fallback_view":
