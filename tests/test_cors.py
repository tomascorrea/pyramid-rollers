...
from pyramid_rollers.service import Service
...
class TestCORS(TestCase):
    ...
    def test_404_returns_CORS_headers(self):
        resp = self.app.get("/bacon/notgood", status=404, headers={"Origin": "notmyidea.org"})
        self.assertIn("Access-Control-Allow-Origin", resp.headers)
    ...
    def test_raise_returns_CORS_headers(self):
        resp = self.app.put("/bacon/raise", status=400, headers={"Origin": "notmyidea.org"})
        self.assertIn("Access-Control-Allow-Origin", resp.headers)
...
class TestAlwaysCORS(TestCase):
    def setUp(self):
        self.config = testing.setUp()
        self.config.include("pyramid_rollers")
        self.config.add_settings({"pyramid_rollers.always_cors": True})
        self.config.add_route("noservice", "/noservice")
        self.config.scan("tests.test_cors")
        self.app = TestApp(CatchErrors(self.config.make_wsgi_app()))
