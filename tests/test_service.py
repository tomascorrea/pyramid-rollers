...
from pyramid_rollers.resource import resource
from pyramid_rollers.service import Service, _UnboundView, clear_services, decorate_view, get_services
from pyramid_rollers.util import func_name
...
class TestService(TestCase):
    ...
    def test_cors_support(self):
        ...
        self.assertTrue(Service(name="foo", path="/foo", cors_enabled=True).cors_enabled)

        self.assertFalse(Service(name="foo", path="/foo", cors_enabled=False).cors_enabled)

        self.assertTrue(Service(name="foo", path="/foo", cors_origins=("*",)).cors_enabled)

        self.assertFalse(
            Service(name="foo", path="/foo", cors_origins=("*"), cors_enabled=False).cors_enabled
        )
    ...
    def test_decorate_view_factory(self):
        args = {"klass": DummyAPI}
        route_args = {"factory": "TheFactoryMethodCalledByPyramid"}

        decorated_view = decorate_view("collection_get", args, "GET", route_args)
