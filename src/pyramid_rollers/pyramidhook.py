...
from pyramid_rollers.cors import (
    CORS_PARAMETERS,
    apply_cors_post_request,
    get_cors_preflight_view,
    get_cors_validator,
)
from pyramid_rollers.errors import Errors
from pyramid_rollers.service import decorate_view
from pyramid_rollers.util import (
...
def apply_filters(request, response):
    if request.matched_route is not None:
        ...
        if service is not None:
            kwargs, ob = getattr(request, "pyramid_rollers_args", ({}, None))
...
def add_nosniff_header(request, response):
    ...
    * https://superevr.com/blog/2012/exploiting-xss-in-ajax-web-applications/
    ...
    Make pyramid-rollers safe-by-default against this attack by including the header.
    """
...
def register_service_views(config, service):
    ...
    services = config.registry.pyramid_rollers_services
    if existing_route:
        route_name = existing_route
        services["__pyramid_rollers" + existing_route] = service
    else:
    ...
    for method, view, args in service.definitions:
        ...
        # (Some of those in pyramid_rollers_parameters, e.g. ``schema``, may contain
        # unpickleable values.)
        for item in args:
            if item not in pyramid_rollers_parameters:
        ...
        for item in pyramid_rollers_parameters:
            if item in args:
                del args[item]
...
def register_resource_views(config, resource):
    ...
    for service in services.values():
        config.add_pyramid_rollers_service(service)
