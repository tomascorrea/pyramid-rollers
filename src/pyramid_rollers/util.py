...
def extract_json_data(request):
    warnings.warn("Use ``pyramid_rollers.validators.extract_cstruct()`` instead", DeprecationWarning)
...
def current_service(request):
    ...
    :rtype: pyramid_rollers.Service
    """
    if request.matched_route:
        services = request.registry.pyramid_rollers_services
        pattern = request.matched_route.pattern
        name = request.matched_route.name
        # try pattern first, then route name else return None
        service = services.get(pattern, services.get("__pyramid_rollers" + name))
        return service
